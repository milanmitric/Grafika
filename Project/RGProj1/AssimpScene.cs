...
namespace RacunarskaGrafika.Vezbe.AssimpNetSample
{
    ...
    /// </summary>
    public class AssimpScene : IDisposable, IDrawable
    {
        #region Atributi
        ...
        private void RenderNode(Node node)
        {
            ...
            // Iscrtavanje pod-dvorova datog cvora.
        ...
        {
            // Pronadji jedinstvene teksture
            HashSet<TextureSlot> texSlots = new HashSet<TextureSlot>();
            ...
            m_texIds = new int[texSlots.Count];
            Gl.glGenTextures(texSlots.Count, m_texIds);

            int index = 0;
            foreach (TextureSlot texSlot in texSlots)
            {
                m_texMappings.Add(texSlot, m_texIds[index]);
                ...
                if (!File.Exists(fileName))
                    throw new ArgumentException("Texture file not found.");
